IRC is used by Electrum MonetaryUnit server to find 'peers' - other Electrum MonetaryUnit servers. The
current list can be seen by running:

  electrum-mue-server peers

The following config file options are used by the IRC part of Electrum MonetaryUnit server:
...
the Electrum MonetaryUnit server will run in private mode (default). In private
mode, `electrum-mue-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum MonetaryUnit
...
your Electrum MonetaryUnit server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-mue .
